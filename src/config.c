...
void loadServerConfigFromString(char *config)
{
    ...
    for (i = 0; i < totlines; i++)
    {
        ...
        {
            if (updateOOMScoreAdjValues(&argv[1], &err, 0) == C_ERR)
                goto loaderr;
        }
...
#define REDIS_CONFIG_REWRITE_SIGNATURE "# Generated by CONFIG REWRITE"
    ...
    void rewriteConfigSdsOption(struct rewriteConfigState * state, const char *option, sds value, const sds defvalue)
    {
        ...
        /* Set force to zero if the value is set to its default. */
        if (defvalue && sdscmp(value, defvalue) == 0)
            force = 0;
...
#define embedConfigInterface(initfn, setfn, getfn, rewritefn) .interface = {.init = (initfn), .set = (setfn), .get = (getfn), .rewrite = (rewritefn)},
