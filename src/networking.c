...
size_t getStringObjectSdsUsedMemory(robj *o) {
    serverAssertWithInfo(NULL, o, o->type == OBJ_STRING); // 断言是字符串对象
    switch (o->encoding) {
        case OBJ_ENCODING_RAW:
            return sdsZmallocSize(o->ptr); // 返回字符串本身的大小
        case OBJ_ENCODING_EMBSTR:
            return zmalloc_size(o) - sizeof(robj); // 整个对象的大小 减去 对象结构的大小 剩下就是字符串的大小
        default:
            return 0; /* Just integer encoding for now. */ // 需要整数编码即可（例如123这种，全部是数字）
    }
}
...
void linkClient(client *c) {
    listAddNodeTail(server.clients, c); // 将客户端添加到全局客户端列表尾部
...
int prepareClientToWrite(client *c) {
    ...
    // 这段代码的功能是检查客户端连接是否为空。如果客户端连接 c->conn 为空，则返回错误码 C_ERR，表示这是一个用于 AOF
    // 加载的虚拟客户端。
    if (!c->conn)
...
void _addReplyProtoToList(client *c, const char *s, size_t len) {
    ...
    if (len) { // 长度还不为0，需要创建新节点
        ...
        tail->size = zmalloc_usable_size(tail) - sizeof(clientReplyBlock); // 总大小
        tail->used = len; // 使用长度
...
void afterErrorReply(client *c, const char *s, size_t len) {
    ...
    if (ctype == CLIENT_TYPE_MASTER || ctype == CLIENT_TYPE_SLAVE ||
        c->id == CLIENT_ID_AOF) { // 主机  或者  从机  或者  AOF端
        char *to, *from;
...
// 状态回复
void addReplyStatus(client *c, const char *status) { addReplyStatusLength(c, status, strlen(status)); }
...
void trimReplyUnusedTailSpace(client *c) {
    ...
    clientReplyBlock *tail = ln ? listNodeValue(ln) : NULL; // 里面的内容是否为空
    ...
    // 为了避免内存重分配后的大规模的移动，我们只对使用部分少的做处理。
        ...
        tail->used < PROTO_REPLY_CHUNK_BYTES) // 使用内存不超过16k
    {
        ...
        // 接管分配的内部内存碎片(至少会用于内存使用跟踪)
        tail->size = zmalloc_usable_size(tail) - sizeof(clientReplyBlock); // 实际分配的空间大小
        c->reply_bytes = c->reply_bytes + tail->size - old_size; // 新的回复总大小
        listNodeValue(ln) = tail; // 指向的尾节点
    }
}
...
void setDeferredArrayLen(client *c, void *node, long length) { setDeferredAggregateLen(c, node, length, '*'); }
...
// 添加 C 字符串数组作为带有标题的状态回复。此函数通常由支持子命令的命令调用，以响应"help"子命令。帮助数组由 NULL
// 哨兵终止。
...
void addReplyFromClient(client *c, client *src) {
    ...
    addReplyProto(c, src->buf, src->bufpos);
    ...
    if (listLength(src->reply))
        listJoin(c->reply, src->reply);
    c->reply_bytes += src->reply_bytes;
    src->reply_bytes = 0;
    src->bufpos = 0;
...
void clientAcceptHandler(connection *conn) {
    ...
    if (server.protected_mode && server.bindaddr_count == 0 && DefaultUser->flags & USER_FLAG_NOPASS &&
...
static void acceptCommonHandler(connection *conn, int flags, char *ip) {
    ...
    if (connGetState(conn) != CONN_STATE_ACCEPTING) {
        serverLog(LL_VERBOSE, "Accepted client connection in error state: %s (conn: %s)", connGetLastError(conn),
    ...
    if ((c = createClient(conn)) == NULL) {
        serverLog(LL_WARNING, "Error registering fd event for the new client: %s (conn: %s)", connGetLastError(conn),
                  connGetInfo(conn, conninfo, sizeof(conninfo)));
    ...
    if (connAccept(conn, clientAcceptHandler) == C_ERR) {
        ...
        if (connGetState(conn) == CONN_STATE_ERROR)
            serverLog(LL_WARNING, "Error accepting a client connection: %s (conn: %s)", connGetLastError(conn),
                      connGetInfo(conn, conninfo, sizeof(conninfo)));
...
void acceptTcpHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while (max--) { // 最多一次接收1000个连接 这里防止一个accept占用太多时间 因为redis是单线程的
                    // 需要给后面的事件让出时间
        // 跟客户端建立通道，为客户端分配一个fd.
...
void freeClientAsync(client *c) {
     ...
     * idle. */
    // 判断是否需要立即关闭客户端连接，判断客户端是否正在执行 Lua 脚本。两种情况下都跳过当前逻辑处理。
    if (c->flags & CLIENT_CLOSE_ASAP || c->flags & CLIENT_LUA)
        return;
    // c->flags 变量按位或上 CLIENT_CLOSE_ASAP 标志
    c->flags |= CLIENT_CLOSE_ASAP;
    // 当只有一个 I/O 线程（即主线程）时，直接将客户端连接 c 添加到 server.clients_to_close 链表尾部，并返回，无需加锁。
 ...
 *
 * 这个方法可能被多线程调用，所以handler_installed参数必须是0，但handler_installed为0时必须保证线程安全。
 */
...
void commandProcessed(client *c) {
    ...
    if (!(c->flags & CLIENT_BLOCKED) || (c->btype != BLOCKED_MODULE && c->btype != BLOCKED_PAUSE)) {
        resetClient(c);
    }
...
// 该函数主要负责调用 processCommand() 并执行一些与客户端相关的子任务：1. 设置当前客户端为 c；2. 如果命令被处理，则调用
// commandProcessed()。 如果客户端在处理命令时被释放，返回 C_ERR，否则返回 C_OK。
...
void readQueryFromClient(connection *conn) {
    ...
    if (c->querybuf_peak < qblen)
        c->querybuf_peak = qblen; // 修改最近读的最大值
    ...
    if (c->flags & CLIENT_MASTER)
        c->read_reploff += nread; // 主机修改复制偏移位置
...
sds catClientInfoString(sds s, client *client) {
    ...
    *p = '\0';
    ...
    return sdscatfmt(
            s,
            "id=%U addr=%s laddr=%s %s name=%s age=%I idle=%I flags=%s db=%i sub=%i psub=%i multi=%i qbuf=%U "
            "qbuf-free=%U argv-mem=%U obl=%U oll=%U omem=%U tot-mem=%U events=%s cmd=%s user=%s redir=%I",
            (unsigned long long) client->id, getClientPeerId(client), getClientSockname(client),
            connGetInfo(client->conn, conninfo, sizeof(conninfo)), client->name ? (char *) client->name->ptr : "",
            (long long) (server.unixtime - client->ctime), (long long) (server.unixtime - client->lastinteraction),
            flags, client->db->id, (int) dictSize(client->pubsub_channels), (int) listLength(client->pubsub_patterns),
            (client->flags & CLIENT_MULTI) ? client->mstate.count : -1, (unsigned long long) sdslen(client->querybuf),
            (unsigned long long) sdsavail(client->querybuf), (unsigned long long) client->argv_len_sum,
...
void securityWarningCommand(client *c) {
    ...
    if (labs(now - logged_time) > 60) {
        serverLog(LL_WARNING, "Possible SECURITY ATTACK detected. It looks like somebody is sending POST or Host: "
                              "commands to Redis. This is likely due to an attacker attempting to use Cross Protocol "
...
void flushSlavesOutputBuffers(void) {
    ...
    while ((ln = listNext(&li))) {
         ...
         */
        if (slave->replstate == SLAVE_STATE_ONLINE && can_receive_writes && !slave->repl_put_online_on_ack &&
...
// IO操作是读还是写
/**
 * io_threads_op 的值为宏定义 IO_THREADS_OP_WRITE：这表明该 IO 线程要做的是写操作，线程会调用 writeToClient
 * 函数将数据写回客户端。 io_threads_op 的值为宏定义 IO_THREADS_OP_READ：这表明该 IO 线程要做的是读操作，线程会调用
 * readQueryFromClient 函数从客户端读取数据。
 */
...
static inline void setIOPendingCount(int i, unsigned long count) { atomicSetWithSync(io_threads_pending[i], count); }
...
int postponeClientRead(client *c) {
    if (server.io_threads_active && // 条件一：全局变量 server 的 io_threads_active 值为 1
        ...
        !(c->flags &
          (CLIENT_MASTER | CLIENT_SLAVE |
           CLIENT_PENDING_READ))) // 条件四：客户端现有标识不能有 CLIENT_MASTER、CLIENT_SLAVE 和 CLIENT_PENDING_READ
    {
        c->flags |= CLIENT_PENDING_READ;
        // 将c放入带读取数据的链表中
...
int handleClientsWithPendingReadsUsingThreads(void) {
    ...
    // 只有在 IO 线程激活，并且 IO 线程可以用于处理待读客户端时，handleClientsWithPendingReadsUsingThreads
    // 函数才会继续执行， 否则该函数就直接结束返回了。
